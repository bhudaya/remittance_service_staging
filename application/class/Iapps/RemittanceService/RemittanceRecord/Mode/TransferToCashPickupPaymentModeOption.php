...
class TransferToCashPickupPaymentModeOption implements RemittancePaymentModeOptionInterface{
    ...
    public function getOption(array $paymentInfo)
    {
        ...
        if( $user = $this->_getUser($this->sender_id) )
        {
            ...
            $paymentInfo['option']['sender_dob'] = $user->getDOB();
            $paymentInfo['option']['sender_gender'] = $user->getGender();
            $paymentInfo['option']['sender_nationality'] = $user->getNationality();
            $paymentInfo['option']['sender_email'] = $user->getEmail();
            $paymentInfo['option']['sender_host_countrycode'] = $user->getHostCountryCode();
            $paymentInfo['option']['sender_host_identity'] = $user->getHostIdentity();
            $paymentInfo['option']['sender_host_identitycard'] = $user->getHostIdentityCard();
            //$paymentInfo['option']['sender_country_dob'] = $user->get;
