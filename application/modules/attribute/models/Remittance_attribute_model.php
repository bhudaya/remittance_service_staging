...
use Iapps\RemittanceService\Attribute\RemittanceAttributeDataMapper;
...
class Remittance_attribute_model extends Base_Model
                           implements RemittanceAttributeDataMapper{

    public function map(\stdClass $data)
    {
        $encrypted_code = EncryptedFieldFactory::build();

        $entity = new RemittanceAttribute();

        if( isset($data->remittance_attribute_id) )
            $entity->setId($data->remittance_attribute_id);
        ...
        if( isset($data->remittance_id) )
            $entity->setRemittanceId($data->remittance_id);
    ...
    {
        $this->db->select('v.id as remittance_attribute_id,
                           v.attribute_id,
                           ...
                           v.deleted_at,
                           v.deleted_by');
        $this->db->from('iafb_remittance.remittance_attribute v');
    ...
    public function findByRemittanceId($remittance_id, $attribute_code = NULL)
    {
        ...
        $this->db->join('iafb_remittance.attribute a', 'v.attribute_id = a.id');
        $this->db->where('v.deleted_at', NULL);
        $this->db->where('v.remittance_id', $remittance_id);
        ...
        if( $query->num_rows () > 0 )
        {
            return $this->mapCollection($query->result(), new RemittanceAttributeCollection(), $query->num_rows());
        }

        return false;
    }
    
    public function insert(RemittanceAttribute $ref_attr)
    {
        $encrypted_code = EncryptedFieldFactory::build();        

        $this->db->set('id', $ref_attr->getId());
        $this->db->set('attribute_id', $ref_attr->getAttribute()->getId());
        $this->db->set('remittance_id', $ref_attr->getRemittanceId());
        ...
        if( $this->db->insert('iafb_remittance.remittance_attribute') )
        {
            return true;
        }
    ...
    public function update(RemittanceAttribute $ref_attr)
    {
        $encrypted_code = EncryptedFieldFactory::build();                
        $this->db->set('other_value', $encrypted_code->setValue($ref_attr->getValue())->getEncodedValue());
        $this->db->set('attribute_value_id', $ref_attr->getAttributeValueId());
        ...
        $this->db->update('iafb_remittance.remittance_attribute');
    ...
    public function delete(RemittanceAttribute $remittance_attribute)
    {
        $deleted_at = IappsDateTime::now()->getUnix();
        $this->db->set('deleted_at', $deleted_at);
        $this->db->set('deleted_by', $remittance_attribute->getDeletedBy());
        $this->db->where('id', $remittance_attribute->getId());

        if( $this->db->update('iafb_remittance.remittance_attribute') )
        {
            $remittance_attribute->getDeletedAt()->setDateTimeUnix($deleted_at);
            return true;
        }
